# IABV v1.5 - Operational Self Examination

Generado: 2026-05-08T03:01:11.728782+00:00
Resumen: Autoexaminacion needs_attention: 23 hallazgos activos. Lo mas fuerte ahora es high_memory_usage. Mejoras validadas: 2 | issues recurrentes: 6.

Usa esta revision para entender que esta fallando, que se repite y que ajustes conviene hacer antes de tocar la arquitectura.

## Hallazgos
- high_memory_usage: El proceso IABV consume 4054MB de RAM. Esto puede causar lentitud en la UI y en respuestas MCP. Considerar lazy loading de servicios no criticos. | recomendacion: Implementar lazy loading: instanciar EmbeddingIndexService, SiteExplorationService, BrowserSessionController y servicios similares solo cuando se usen por primera vez, no en __init__. Usar @property con cache en AppBootstrap. | confianza 0.95
- UI event loop stalls: 2 detected (worst 493600ms): 2 heartbeat stalls detected. Worst: 493600ms. Dominant phase: startup_background:truth_refresh. | recomendacion: Investigate blocking on main thread during phase "startup_background:truth_refresh". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | confianza 0.95
- network_probe_failing: El probe de conectividad falla: <urlopen error [Errno 11001] getaddrinfo failed>. Esto bloquea al WorldModel scan por hasta 7s cada ciclo y causa que IABV reporte "sin internet" incorrectamente. | recomendacion: Verificar firewall: port 53 TCP puede estar bloqueado. El probe ya tiene fallback a port 443 y HTTP HEAD. Si todos fallan, verificar proxy/VPN. Considerar aumentar NETWORK_CACHE_TTL a 120s para reducir impacto. | confianza 0.90
- RSS crecio 2772MB durante startup: RSS paso de 103MB (bootstrap_init_start) a 2875MB (lazy_vm_prebuild_paused), un crecimiento de 2772MB (umbral 150MB). Esto puede causar presion de memoria y GC stalls. | recomendacion: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | confianza 0.90
- Tendencia de degradacion detectada: La tasa de exito cayo de 97% a 73% (delta=-23%). Revisar cambios recientes en configuracion o entorno. | recomendacion: sin ajuste concreto | confianza 0.00
- multi_source_disagreement repetido en logs: Detectadas 10 ocurrencias de "multi_source_disagreement" en las ultimas 500 lineas del log. Ejemplo reciente: 2026-05-07 21:52:41,354 | INFO | iabv_v15.services.tools.tool_adapters | multi_source_disagreement: windsurf_installed — positives=['filesystem'] negatives=['process', 'window'] | La herramienta exist | recomendacion: El cache de 300s puede no ser suficiente o el MCP polling recrea instancias que pierden el cache. Considerar aumentar TTL o mover cache a nivel de clase persistente. | confianza 0.90

## Ajustes recomendados
- high_memory_usage: Implementar lazy loading: instanciar EmbeddingIndexService, SiteExplorationService, BrowserSessionController y servicios similares solo cuando se usen por primera vez, no en __init__. Usar @property con cache en AppBootstrap. | fuente runtime_performance_monitor
- UI event loop stalls: 2 detected (worst 493600ms): Investigate blocking on main thread during phase "startup_background:truth_refresh". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | fuente n/d
- network_probe_failing: Verificar firewall: port 53 TCP puede estar bloqueado. El probe ya tiene fallback a port 443 y HTTP HEAD. Si todos fallan, verificar proxy/VPN. Considerar aumentar NETWORK_CACHE_TTL a 120s para reducir impacto. | fuente WorldModelService._network_connectivity_probe
- RSS crecio 2772MB durante startup: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | fuente data/logs/startup_timeline.jsonl
- multi_source_disagreement repetido en logs: El cache de 300s puede no ser suficiente o el MCP polling recrea instancias que pierden el cache. Considerar aumentar TTL o mover cache a nivel de clase persistente. | fuente runtime_log, C:\Python\IABV_v1.5\data\logs\iabv_v15.log
- Herramienta faltante reportada en logs: Verificar si la herramienta faltante es necesaria para el flujo actual o si existe un fallback disponible. | fuente runtime_log, C:\Python\IABV_v1.5\data\logs\iabv_v15.log

## Mejoras validadas
- cloud_provider por cloud: Recomiendo cloud para cloud_reasoning en cloud_provider:groq usando cloud_provider con configuracion cloud:groq por score base 0.79 y score adaptativo 1.09 con 21 muestra(s). Ajuste adaptativo: historial de exito 100%; reutilizacion alta 100%. | confianza 1.00
- Promocion validada en sandbox: No hay propuestas ni recomendaciones candidatas para validar; el ciclo autonomo esta al dia. | confianza 0.82

## Retroalimentacion de ajustes
- multi_source_disagreement repetido en logs: no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
- Ruido excesivo de logs HTTP (httpx): no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
- RSS crecio 2528MB durante startup: no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
- Herramienta faltante reportada en logs: no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.

UNRESOLVED: UNRESOLVED:network_latency