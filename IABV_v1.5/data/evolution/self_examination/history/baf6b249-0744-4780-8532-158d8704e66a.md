...
Generado: 2026-05-06T22:39:42.272929+00:00
...
## Hallazgos
- high_memory_usage: El proceso IABV consume 4385MB de RAM. Esto puede causar lentitud en la UI y en respuestas MCP. Considerar lazy loading de servicios no criticos. | recomendacion: Implementar lazy loading: instanciar EmbeddingIndexService, SiteExplorationService, BrowserSessionController y servicios similares solo cuando se usen por primera vez, no en __init__. Usar @property con cache en AppBootstrap. | confianza 0.95
- UI event loop stalls: 3 detected (worst 111538ms): 3 heartbeat stalls detected. Worst: 111538ms. Dominant phase: unknown. | recomendacion: Investigate blocking on main thread during phase "unknown". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | confianza 0.95
- Ruido excesivo de logs HTTP (httpx): Detectadas 18 ocurrencias de "HTTP Request:" en las ultimas 500 lineas del log. Ejemplo reciente: 2026-05-06 17:39:15,861 | INFO | httpx | HTTP Request: GET http://127.0.0.1:11434/api/tags "HTTP/1.1 200 OK" | recomendacion: Demasiadas lineas de httpx poluciona el log y dificulta encontrar hallazgos importantes. Auto-suprimir httpx a WARNING cuando exceda el umbral. | confianza 0.90
...
- UI event loop stalls: 3 detected (worst 111538ms): Investigate blocking on main thread during phase "unknown". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | fuente n/d
...
- UI event loop stalls: 1 detected (worst 3036ms): no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
