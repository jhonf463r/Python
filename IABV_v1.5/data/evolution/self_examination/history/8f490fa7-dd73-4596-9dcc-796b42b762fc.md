...
Generado: 2026-05-07T02:45:54.805687+00:00
...
- RSS crecio 2189MB durante startup: RSS paso de 98MB (bootstrap_init_start) a 2288MB (lazy_vm_knowledge_done), un crecimiento de 2189MB (umbral 150MB). Esto puede causar presion de memoria y GC stalls. | recomendacion: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | confianza 0.90
...
- UI event loop stalls: 4 detected (worst 472817ms): Investigate blocking on main thread during phase "unknown". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | fuente n/d
...
- RSS crecio 2189MB durante startup: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | fuente data/logs/startup_timeline.jsonl
...
- RSS crecio 960MB durante startup: no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
