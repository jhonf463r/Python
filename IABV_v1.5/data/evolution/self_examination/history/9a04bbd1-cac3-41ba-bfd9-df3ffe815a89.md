...
Generado: 2026-05-08T15:43:50.154574+00:00
...
- UI event loop stalls: 1 detected (worst 2030ms): 1 heartbeat stalls detected. Worst: 2030ms. Dominant phase: prebuild_waiting:startup_background_active:startup_truth_refresh. | recomendacion: Investigate blocking on main thread during phase "prebuild_waiting:startup_background_active:startup_truth_refresh". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | confianza 0.95
- Herramienta faltante reportada en logs: Detectadas 6 ocurrencias de "tool_missing" en las ultimas 500 lineas del log. Ejemplo reciente: 2026-05-08 10:43:05,173 | INFO | iabv_v15.bootstrap | tool_missing: aider_coder — adapter=aider | fix: pip install aider-chat (optional, heavy ~200MB; installed in background) | recomendacion: Verificar si la herramienta faltante es necesaria para el flujo actual o si existe un fallback disponible. | confianza 0.90
...
- RSS crecio 3412MB durante startup: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | fuente data/logs/startup_timeline.jsonl
- UI event loop stalls: 1 detected (worst 2030ms): Investigate blocking on main thread during phase "prebuild_waiting:startup_background_active:startup_truth_refresh". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | fuente n/d
...
- RSS crecio 3412MB durante startup: no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
- UI event loop stalls: 1 detected (worst 2030ms): no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
