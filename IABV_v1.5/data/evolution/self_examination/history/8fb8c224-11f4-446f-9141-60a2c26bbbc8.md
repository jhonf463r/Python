...
Generado: 2026-05-06T23:20:32.810127+00:00
...
## Hallazgos
- high_memory_usage: El proceso IABV consume 2917MB de RAM. Esto puede causar lentitud en la UI y en respuestas MCP. Considerar lazy loading de servicios no criticos. | recomendacion: Implementar lazy loading: instanciar EmbeddingIndexService, SiteExplorationService, BrowserSessionController y servicios similares solo cuando se usen por primera vez, no en __init__. Usar @property con cache en AppBootstrap. | confianza 0.95
- UI event loop stalls: 24 detected (worst 1440324ms): 24 heartbeat stalls detected. Worst: 1440324ms. Dominant phase: unknown. | recomendacion: Investigate blocking on main thread during phase "unknown". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | confianza 0.95
...
- RSS crecio 464MB durante startup: RSS paso de 1691MB (bridge_chat_queued) a 2155MB (window_activeChanged), un crecimiento de 464MB (umbral 150MB). Esto puede causar presion de memoria y GC stalls. | recomendacion: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | confianza 0.90
...
- UI event loop stalls: 24 detected (worst 1440324ms): Investigate blocking on main thread during phase "unknown". Consider moving heavy work to background threads or adding yield-to-event-loop calls. | fuente n/d
...
- RSS crecio 464MB durante startup: Revisar que ViewModels con defer_initial_refresh=True no hagan queries pesados en el constructor. Verificar que _log_tool_availability() no cree objetos grandes. | fuente data/logs/startup_timeline.jsonl
...
## Mejoras validadas
- cloud_provider por cloud: Recomiendo cloud para cloud_reasoning en cloud_provider:groq usando cloud_provider con configuracion cloud:groq por score base 0.78 y score adaptativo 1.08 con 5 muestra(s). Ajuste adaptativo: historial de exito 100%; reutilizacion alta 80%. | confianza 1.00
...
- UI event loop stalls: 20 detected (worst 16424ms): no_evidence | Todavia no hay suficiente evidencia posterior para juzgar si esta recomendacion sirvio o no. | siguiente paso: Mantenerla en observacion hasta tener mas corridas comparables.
