...
class ControlMasterService:
    ...
    def __init__(
        ...
        *,
        ...
        structured_need_repository: Any | None = None,
        ...
        self.structured_need_repository = structured_need_repository
    ...
    def current_work_queue(self, *, limit: int = 20) -> list[dict[str, Any]]:
        ...
        OSES findings (HIGH/CRITICAL), structured needs (PENDING),
        and failed/stalled interaction episodes from ``runtime_audit.jsonl``.
        Each item carries a deterministic ``priority_score`` with ``score_breakdown``.
        ...
        # 6. Structured needs (PENDING only)
        items.extend(self._project_structured_need_items(seen_ids))
    ...
    def _project_structured_need_items(
        self, seen_ids: set[str],
    ) -> list[dict[str, Any]]:
        """Project pending StructuredNeeds into the work queue.

        Only PENDING needs are projected. Selection is deterministic based
        on priority field. Provenance is preserved via source_finding_id.
        """
        if self.structured_need_repository is None:
            return []
        items: list[dict[str, Any]] = []
        try:
            from iabv_v15.domain.models import NeedStatus as _NS
            pending_needs = self.structured_need_repository.list_pending()
        except Exception:
            return []
        for need in pending_needs:
            nid = f'need:{need.need_id}'
            if nid in seen_ids:
                continue
            seen_ids.add(nid)
            score: dict[str, int] = {'base': 18}
            prio_map = {'critical': 100, 'high': 75, 'medium': 50, 'low': 25}
            score['priority'] = prio_map.get(need.priority, 30)
            evidence_refs = []
            if need.source_finding_id:
                evidence_refs.append(need.source_finding_id)
            items.append(self._make_item(
                item_id=nid,
                title=need.capability_gap or 'Structured need',
                status=need.status.value,
                source='structured_need_repository',
                evidence_refs=evidence_refs,
                next_action=need.desired_state or 'Investigate capability gap',
                score_components=score,
                updated_at=need.created_at.isoformat() if hasattr(need.created_at, 'isoformat') else '',
