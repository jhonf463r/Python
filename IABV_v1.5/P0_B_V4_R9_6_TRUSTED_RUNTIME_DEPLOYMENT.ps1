# P0-B V4-R9.6 Trusted Runtime Deployment
...
Write-Output "=== P0-B V4-R9.6 TRUSTED RUNTIME DEPLOYMENT ==="
Write-Output ""
...
# PHASE 2: Verify source runtime ACLs
Write-Output "=== PHASE 2: VERIFY SOURCE RUNTIME ACLS ==="
...
# PHASE 3: Install required packages in trusted source
Write-Output "=== PHASE 3: INSTALL REQUIRED PACKAGES IN TRUSTED SOURCE ==="
Write-Output "Installing pywin32..."
& "$trustedSource\python.exe" -m pip install pywin32 --no-warn-script-location
Write-Output "Installing cryptography..."
& "$trustedSource\python.exe" -m pip install cryptography --no-warn-script-location
Write-Output "Required packages installed in trusted source"
Write-Output ""

# PHASE 4: Verify repository state
Write-Output "=== PHASE 4: VERIFY REPOSITORY STATE ==="
...
# PHASE 5: Check for pythonservice.exe in trusted source
Write-Output "=== PHASE 5: CHECK FOR PYTHONSERVICE.EXE ==="
$pythonserviceInSource = Test-Path "$trustedSource\Scripts\pythonservice.exe"
if ($pythonserviceInSource) {
    Write-Output "pythonservice.exe found in trusted source: $trustedSource\Scripts\pythonservice.exe"
} else {
    Write-Output "WARNING: pythonservice.exe not found in trusted source Scripts"
    Write-Output "Will use pythonservice.exe from site-packages after deployment"
}
Write-Output ""

# PHASE 6: Remove existing service
Write-Output "=== PHASE 6: REMOVE EXISTING SERVICE ==="
try {
...
# PHASE 7: Create machine-scoped runtime directory
Write-Output "=== PHASE 7: CREATE MACHINE-SCOPED RUNTIME DIRECTORY ==="
...
# PHASE 8: Copy Python runtime components from trusted source
Write-Output "=== PHASE 8: COPY PYTHON RUNTIME COMPONENTS FROM TRUSTED SOURCE ==="
...
# PHASE 9: Copy IABV service modules
Write-Output "=== PHASE 9: COPY IABV SERVICE MODULES ==="
...
# PHASE 10: Configure ACLs for machine-scoped runtime
Write-Output "=== PHASE 10: CONFIGURE MACHINE-SCOPED RUNTIME ACLS ==="
...
# PHASE 11: Verify ACLs
Write-Output "=== PHASE 11: VERIFY ACLS ==="
icacls $serviceRuntimePath
Write-Output ""

# PHASE 12: Determine pythonservice.exe path
...
# PHASE 13: Install service with machine-scoped PathName
Write-Output "=== PHASE 13: INSTALL SERVICE WITH MACHINE-SCOPED PATHNAME ==="
...
# PHASE 14: Verify service installation
Write-Output "=== PHASE 14: VERIFY SERVICE INSTALLATION ==="
...
# PHASE 15: Verify StartName is LocalService
Write-Output "=== PHASE 15: VERIFY SERVICE IDENTITY ==="
...
# PHASE 16: Verify PathName points to machine-scoped runtime
Write-Output "=== PHASE 16: VERIFY EXECUTION BOUNDARY ==="
...
# PHASE 17: Verify PathName does NOT point to user profile
Write-Output "=== PHASE 17: VERIFY NO USER-PROFILE DEPENDENCY ==="
...
# PHASE 19: Verify ACLs on critical components
Write-Output "=== PHASE 19: VERIFY CRITICAL COMPONENT ACLS ==="
$criticalPaths = @(
    "$serviceRuntimePath\python.exe",
    "$pythonservicePath",
