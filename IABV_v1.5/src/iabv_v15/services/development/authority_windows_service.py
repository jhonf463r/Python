...
class AuthorityKeyManager:
    ...
    - Self-provisioning mode for initial setup (service generates key under its own identity)
    ...
    def __init__(
        ...
        Args:
            ...
            allow_provisioning: If True, allow key generation during load if missing.
                               This must be True only during authorized provisioning,
                               and False during normal runtime to enforce fail-closed.
        """
        ...
        self.allow_provisioning = allow_provisioning
        ...
        logger.info("AuthorityKeyManager initialized: %s (provisioning=%s)", key_storage_path, allow_provisioning)
    ...
    def load_key(self) -> None:
        ...
        If allow_provisioning=True and key is missing, generates key under current identity.
        If allow_provisioning=False (normal runtime), fails closed if key is missing.
...
if not WINDOWS_SERVICE_AVAILABLE:
    ...
    class AuthorityServiceHandler(win32serviceutil.ServiceFramework):
        ...
        def SvcDoRun(self):
            ...
            Supports provisioning mode via IABV_AUTHORITY_PROVISIONING=1.
            """
            ...
            # F14 V4-r9.4 FIX: Check provisioning mode
            allow_provisioning = os.environ.get("IABV_AUTHORITY_PROVISIONING", "0") == "1"
            if allow_provisioning:
                logger.warning("Running in PROVISIONING MODE - key will be generated under current process identity")
            ...
            try:
                ...
                logger.info("Key manager initialized (provisioning=%s)", allow_provisioning)
                
                # Step 2: Load authority key (fail-closed if missing unless provisioning)
                try:
                    ...
                    if allow_provisioning:
                        logger.error("Provisioning mode failed - check permissions and service identity")
                    else:
