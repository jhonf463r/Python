...
def _load_runtime_organ_state(workspace: Path) -> dict[str, Any]:
    ...
    try:
        if path.exists():
            ...
            return {"status": "ok", "organ_count": len(data.get("organs", [])), "data": data}
        return {"status": "unavailable", "organ_count": 0}
    except Exception as e:
        return {"status": "error", "error": str(e), "organ_count": 0}
...
def _load_portable_context_summary(workspace: Path) -> dict[str, Any]:
    ...
    This is read-only - does not modify any state.
    """
    # Try to load from agent_session_gate history
    ...
    try:
        if history_dir.exists():
            ...
            if history_files:
                ...
                return {"status": "ok", "history_files": len(history_files), "latest_file": latest.name, "data": data}
