...
def test_real_build_review_with_production_dependencies():
    ...
    try:
        ...
        # FORENSIC: Capture full findings vs returned findings
        print(f"FORENSIC_FULL_FINDINGS_COUNT: {len(findings)}")
        print(f"FORENSIC_RETURNED_FINDINGS_COUNT: {len(review.findings)}")
        print(f"FORENSIC_TRUNCATION_APPLIED: {len(findings) != len(review.findings)}")
        print()
