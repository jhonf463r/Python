"""
Tests for DevelopmentAuditResult model.
...
- Empty execution_evidence_id rejection
- DEVELOPMENT_EXECUTION EvidenceRef coherence
- Multiple DEVELOPMENT_EXECUTION refs rejection
- Mismatched execution_evidence_id vs EvidenceRef rejection
- No findings on PASS
- Findings on FAIL
- Explanatory findings on INCONCLUSIVE
- JSON round-trip persistence
- Nullable auditor_id
- Timestamp preservation
- Criteria persistence
- Metadata persistence
- Real evidence graph linkage
"""
...
class TestDevelopmentAuditFinding:
    ...
    def test_finding_with_all_fields(self):
        ...
        assert finding.summary == "Critical finding"
        assert finding.criterion == "test_coverage"
        assert len(finding.evidence_refs) == 2
        assert finding.metadata["source"] == "manual"
...
class TestDevelopmentAuditCriterion:
    ...
    def test_minimal_criterion(self):
        """Criterion can be created with minimal fields."""
    ...
    def test_criterion_with_all_fields(self):
        """Criterion can be created with all fields."""
        criterion = DevelopmentAuditCriterion(
            criterion_id="criterion-1",
            name="Test Coverage",
            description="All critical paths must have tests",
            required=True,
            status="satisfied",
            evidence_refs=["test-1", "test-2"],
            metadata={"category": "quality"},
        )
        assert criterion.description == "All critical paths must have tests"
        assert criterion.status == "satisfied"
        assert len(criterion.evidence_refs) == 2


class TestDevelopmentAuditResultMinimal:
    """Test minimal audit result construction."""

    def test_minimal_pass_result(self):
        """PASS audit result can be created with minimal fields."""
    ...
    def test_minimal_fail_result(self):
        """FAIL audit result can be created with minimal fields."""
        audit_result = DevelopmentAuditResult(
            execution_evidence_id="exec-evidence-123",
    ...
    def test_minimal_inconclusive_result(self):
        """INCONCLUSIVE audit result can be created with minimal fields."""
        audit_result = DevelopmentAuditResult(
            execution_evidence_id="exec-evidence-123",
...
class TestDevelopmentAuditResultComplete:
    """Test complete audit result construction."""

    def test_complete_pass_result(self):
        """PASS audit result can be created with all fields."""
        audit_result = DevelopmentAuditResult(
            execution_evidence_id="exec-evidence-123",
            verdict=DevelopmentAuditVerdict.PASS,
...
class TestDevelopmentAuditResultTargetLinkage:
    """Test target evidence linkage."""

    def test_execution_evidence_id_valid(self):
        """Valid execution_evidence_id is accepted."""
        audit_result = DevelopmentAuditResult(
            execution_evidence_id="exec-evidence-123",
            verdict=DevelopmentAuditVerdict.PASS,
        )
    ...
    def test_empty_execution_evidence_id_rejected(self):
        ...
        with pytest.raises(ValueError, match="execution_evidence_id cannot be empty"):
            DevelopmentAuditResult(
...
class TestDevelopmentAuditResultEvidenceRefCoherence:
    """Test EvidenceRef coherence with execution_evidence_id."""

    def test_matching_development_exec_ref(self):
        """DEVELOPMENT_EXECUTION EvidenceRef matching execution_evidence_id is valid."""
        audit_result = DevelopmentAuditResult(
            execution_evidence_id="exec-evidence-123",
            verdict=DevelopmentAuditVerdict.PASS,
            evidence_refs=[
        ...
        assert audit_result.evidence_refs[0].ref_id == "exec-evidence-123"

    def test_mismatched_development_exec_ref_rejected(self):
        """DEVELOPMENT_EXECUTION EvidenceRef not matching execution_evidence_id is rejected."""
        with pytest.raises(ValueError, match="does not match execution_evidence_id"):
            DevelopmentAuditResult(
                execution_evidence_id="exec-evidence-123",
                verdict=DevelopmentAuditVerdict.PASS,
                evidence_refs=[
    ...
    def test_multiple_development_exec_refs_rejected(self):
        ...
        with pytest.raises(ValueError, match="Multiple DEVELOPMENT_EXECUTION EvidenceRefs"):
            DevelopmentAuditResult(
                execution_evidence_id="exec-evidence-123",
                verdict=DevelopmentAuditVerdict.PASS,
                evidence_refs=[
