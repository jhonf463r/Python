...
def test_user_browser_handoff_when_user_sees_chatgpt() -> None:
    ...
    assert stub._live_status == 'idle'
    assert 'external_failure_followup' in stub._latest_response_meta
    assert stub._external_consultation_browser_override.get('mode') == 'user_browser_manual'
    assert 'navegador del usuario' in stub._latest_response_text or 'Chrome/navegador normal' in stub._latest_response_text
...
def test_platform_pending_p027_json_validates() -> None:
    ...
    assert task.status.value in {'PENDING', 'COMPLETED'}
