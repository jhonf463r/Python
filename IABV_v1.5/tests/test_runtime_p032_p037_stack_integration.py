...
class TestUserChromeAvailableWhenP032Present:
    ...
    def test_bridge_activates_with_cdp_available(self, viewmodel_cls):
        ...
        vm._detect_cdp_available = MagicMock(return_value=True)
...
class TestUserChromeNotPromisedWithoutP032:
    ...
    def test_guard_blocks_when_no_security_failure(self, viewmodel_cls):
        """Without a recent security_verification failure, bridge selection returns False."""
        vm = MagicMock(spec=viewmodel_cls)
        vm._USER_CHROME_BRIDGE_PATTERNS = viewmodel_cls._USER_CHROME_BRIDGE_PATTERNS
        vm._last_external_failure_payload = {}
    ...
    def test_available_actions_no_cdp_without_bridge(self, viewmodel_cls):
        if not hasattr(viewmodel_cls, '_describe_available_actions'):
            pytest.skip('_describe_available_actions not present in this build')
...
class TestSecurityBlockFollowupNoLocalFallback:
    ...
    def test_help_offer_captured_by_incident_handler(self, viewmodel_cls, active_incident):
        if not hasattr(viewmodel_cls, '_try_handle_incident_followup'):
            pytest.skip('P0.37 _try_handle_incident_followup not present in this build')
        vm = MagicMock(spec=viewmodel_cls)
    ...
    def test_en_que_te_ayudo_not_local(self, viewmodel_cls, active_incident):
        if not hasattr(viewmodel_cls, '_classify_incident_followup_intent'):
            pytest.skip('P0.37 _classify_incident_followup_intent not present')
...
class TestIncidentWindowFocusUsesWorldModel:
    ...
    def test_focus_uses_hwnd_from_incident(self, viewmodel_cls):
        if not hasattr(viewmodel_cls, '_try_focus_incident_window'):
            pytest.skip('P0.37 _try_focus_incident_window not present')
        incident = {
