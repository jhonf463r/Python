...
def test_sync_applies_ff_pull_when_safe() -> None:
    recipe = {
        ...
        ("git", "status", "--porcelain"): FakeCompleted(stdout=""),
        ("git", "rev-list", "--left-right", "--count", "HEAD...origin/main"): FakeCompleted(stdout="0 2\n"),
        ("git", "pull", "--no-rebase", "origin", "main"): FakeCompleted(stdout="Fast-forward\n"),
    ...
    assert ("git", "pull", "--no-rebase", "origin", "main") in runner.calls
...
def test_sync_up_to_date_does_not_register_unresolved() -> None:
    ...
    assert ("git", "pull", "--no-rebase", "origin", "main") not in runner.calls
...
def test_sync_reports_pull_failure() -> None:
    recipe = {
        ...
        ("git", "status", "--porcelain"): FakeCompleted(stdout=""),
        ("git", "rev-list", "--left-right", "--count", "HEAD...origin/main"): FakeCompleted(stdout="0 1\n"),
        ("git", "pull", "--no-rebase", "origin", "main"): FakeCompleted(stderr="not a fast-forward\n", returncode=1),
    }
    svc, _ = _svc(recipe)
    result = svc.sync()
    assert result.applied is False
