"""
P0.213 V5 Invocation Authority Adversarial Tests

Tests for the invocation authority contract implementation.
...
from iabv_v15.services.evolution.capability_verifier import CapabilityVerifier, CapabilityVerificationResult
...
def test_f01_selfaudit_without_capability_fail_closed():
    """
    F-01: SelfAuditService must reject calls without validated invocation context.
    
    P0.213 V5: SelfAuditService requires validated_invocation_context.
...
def test_f01_selfaudit_with_invalid_capability_fail_closed():
    """
    F-01: SelfAuditService must reject invalid validated invocation context.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        # Setup
        tool_registry = MagicMock(spec=ToolRegistry)
        ...
        with pytest.raises(ValueError, match="Missing required field"):
            self_audit.run(
                reason="test",
                validated_invocation_context={"capability_id": "test"}
            )
        
        # Test: Invalid verifier_signature
        with pytest.raises(ValueError, match="Invalid verifier_signature"):
            self_audit.run(
                reason="test",
                validated_invocation_context={
                    ...
                    "verifier_signature": 123,  # Not a string
                }
            )
...
def test_f02_parent_authorized_child_pass():
    """
    F-02: Parent issues capability to authorized child → PASS.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        # Setup
...
def test_f03_same_lineage_unauthorized_consumer_fail():
    """
    F-03: Same lineage but unauthorized consumer → FAIL.
...
def test_f04_valid_ipc_wrong_capability_consumer_fail():
    """
    F-04: Valid IPC connection but wrong capability consumer → FAIL.
...
def test_f05_same_user_dacl_unauthorized_process_fail():
    """
    F-05: Same-user DACL allows connection but unauthorized process → FAIL.
...
def test_a_to_a_pass():
    """Authorized consumer A uses capability issued to A → PASS."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage_root = Path(tmpdir)
        issuer = CapabilityIssuer(
            issuer_pid=os.getpid(),
...
def test_a_to_b_fail():
    """Authorized consumer A uses capability issued to B → FAIL."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage_root = Path(tmpdir)
        issuer = CapabilityIssuer(
            issuer_pid=os.getpid(),
...
def test_replay_fail():
    """Replay of same capability → FAIL."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage_root = Path(tmpdir)
        issuer = CapabilityIssuer(
            issuer_pid=os.getpid(),
...
def test_runtime_n_to_n_plus_1_fail():
    """Capability from runtime N used in runtime N+1 → FAIL."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage_root = Path(tmpdir)
        
        # Issuer in runtime N
        issuer_n = CapabilityIssuer(
            issuer_pid=os.getpid(),
            secret_key=b"test_secret",
            runtime_incarnation=1,
        )
        
        # Verifier in runtime N+1
        issuer_n_plus_1 = CapabilityIssuer(
            issuer_pid=os.getpid(),
            secret_key=b"test_secret",
...
def test_wrong_scope_fail():
    """Capability issued for scope A used for scope B → FAIL."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage_root = Path(tmpdir)
        issuer = CapabilityIssuer(
            issuer_pid=os.getpid(),
