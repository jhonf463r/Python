...
class TestCDPProbeAvailable:
    """User has Chrome running with --remote-debugging-port."""
    ...
    def test_cdp_available_returns_true(self, viewmodel_cls):
        ...
        with patch.dict(os.environ, {'IABV_SHARED_CDP_URL': 'http://localhost:9222'}):
            with patch('urllib.request.urlopen') as mock_urlopen:
                resp = MagicMock()
                resp.read.return_value = version_json
                resp.__enter__ = lambda s: s
                resp.__exit__ = MagicMock(return_value=False)
                mock_urlopen.return_value = resp
                result = viewmodel_cls._detect_cdp_available(vm)
...
class TestPermissionDenied:
    ...
    def test_non_matching_message_returns_false(self, viewmodel_cls):
        vm = MagicMock(spec=viewmodel_cls)
        vm._USER_CHROME_BRIDGE_PATTERNS = viewmodel_cls._USER_CHROME_BRIDGE_PATTERNS
...
class TestManualSelectionRequired:
    ...
    def test_cdp_unavailable_shows_manual_instructions(self, viewmodel_cls):
        ...
        vm._assistant_display_name = lambda kind: kind.title()
...
class TestNoPIILeak:
    ...
    def test_session_selection_message_no_pii(self, viewmodel_cls):
        vm = MagicMock(spec=viewmodel_cls)
        result = viewmodel_cls._build_session_selection_message(vm, 'ChatGPT', cdp_available=True)
        msg = result['message']
...
class TestExternalConsultationNoSilentFallback:
    ...
    def test_security_verification_mentions_isolated_profile(self, viewmodel_cls):
        vm = MagicMock(spec=viewmodel_cls)
        result = viewmodel_cls._build_session_selection_message(
            vm, 'ChatGPT', cdp_available=False,
        )
        msg = result['message']
        assert 'perfil aislado' in msg.lower() or 'chatgpt_program_session' in msg
...
class TestOSESIsolatedProfileFinding:
    ...
    def test_finding_generated_when_blocks_ge_2(self, tmp_path):
        ...
        assert findings[0].metadata['profile_block_count'] >= 2
...
class TestTracerBridgeEvents:
    ...
    def test_session_selected_event(self, tracer):
        ...
        assert ev['data']['selected_session'] == 'user_chrome_cdp'
    ...
    def test_bridge_result_cdp_unavailable(self, tracer):
        ...
        assert ev['kind'] == 'user_browser_bridge_result'
    ...
    def test_permission_denied_event(self, tracer):
        ...
        assert ev['kind'] == 'user_browser_bridge_result'
        assert ev['data']['selected_session'] == 'isolated_profile'
...
class TestSessionSelectionMessageCDPAvailable:
    ...
    def test_message_includes_user_chrome_option(self, viewmodel_cls):
        ...
        result = viewmodel_cls._build_session_selection_message(vm, 'ChatGPT', cdp_available=True)
        msg = result['message']
        assert 'usar mi chrome' in msg.lower() or 'chrome' in msg.lower()
...
class TestPlatformPendingP032:
    ...
    def test_p032_json_validates(self):
        ...
        found = False
        for candidate in [
            p / 'task_governed_user_chrome_bridge_p032.json',
            p / 'task_runtime_governed_user_chrome_bridge_p032.json',
            Path(__file__).resolve().parent.parent / 'data' / 'evolution' / 'platform_pending' / 'task_governed_user_chrome_bridge_p032.json',
        ]:
            if candidate.exists():
        ...
        if not found:
            ...
            if ws.is_dir():
                pp = ws / 'data' / 'evolution' / 'platform_pending' / 'task_governed_user_chrome_bridge_p032.json'
                if pp.exists():
