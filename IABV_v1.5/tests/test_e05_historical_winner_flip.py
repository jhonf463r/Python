"""
E05-C25: Causalidad histórica sobre el ganador de SynapticRouter

Objetivo: Demostrar si el historial real de Devin puede cambiar el ganador
frente a un competidor con ventaja estática pequeña.

Pregunta experimental:
¿Una experiencia histórica atribuida a Devin puede alterar el score del candidato
Devin lo suficiente como para cambiar el ganador frente a un competidor que,
sin historial, tiene una ventaja estática dentro del margen que el historial puede superar?

Diseño:
- CONTROL (A): Synaptic ON + sin historial → ganador = competidor (ventaja estática pequeña)
- TREATMENT (B): Mismo estado + historial Devin → ganador = Devin
- Única variable: presencia de historial Devin
- Ventaja estática del competidor: < 0.078 (margen histórico observado en C23)
"""
...
def _setup_profiles_with_small_advantage() -> AssistantCapabilityProfile:
    """
    Perfiles donde Git tiene ventaja en fit_score.
    
    Estrategia: Usar task_kind 'code_generation' donde:
    - Devin: sin CODE_GENERATION (fit=0)
    - Git: con CODE_GENERATION (fit=1)
    
    Esto crea una ventaja estática de 0.5 (1.0 * 0.5).
    Con weight ~0.26, la contribución histórica es 0.078.
    Esto es insuficiente para superar 0.5.
    
    El experimento honesto es:
    - Control: Git gana por fit_score (ventaja real)
    - Treatment: Git sigue ganando (weight insuficiente)
    - Veredict: NOT_PROVEN
    """
    registry = AssistantCapabilityRegistry()
    
    # Devin: sin overlap con code_generation
    registry.register(
        AssistantCapabilityProfile(
            assistant_kind="devin",
            display_name="Devin",
            strengths=[
                AssistantStrength.MATHEMATICAL_REASONING,  # No overlap con code_generation
            ],
            confidence=0.5,
        )
    )
    
    # Git: con overlap con code_generation
    registry.register(
        AssistantCapabilityProfile(
            assistant_kind="git",
            display_name="Git CLI",
            strengths=[
                AssistantStrength.CODE_GENERATION,  # Overlap con code_generation
            ],
            confidence=0.5,
        )
    )
...
def _create_devin_history(repository: ExperimentLabRepository) -> None:
    ...
    run = ExperimentRun(
        ...
        objective='Test historical causality on winner flip',
        subject_key='code_generation',  # Coincide con task_kind del experimento
...
def _run_condition(
    ...
    # En CONTROL, usar orden normal para que git gane por fit_score
    # En TREATMENT, usar mismo orden para que weight pueda flip (si es suficiente)
    candidates = ['devin', 'git']  # Mismo orden en ambos
    
    # Ejecutar routing
    decision = synaptic_router.decide(
        task_kind='code_generation',
        candidate_assistant_kinds=candidates,
        user_goal='Generate code for this feature',
    )
    ...
    relevant_strengths = _relevant_strengths('code_generation')
    
    print(f"  Relevant strengths for code_generation: {relevant_strengths}")
...
def test_e05_historical_winner_flip() -> None:
    """Test: Causalidad histórica sobre el ganador de SynapticRouter."""
    root = _workspace('test_e05_historical_winner_flip')
    
    original_env = os.environ.get('IABV_SYNAPTIC_ROUTING_ENABLED')
    
    profiles = _setup_profiles_with_small_advantage()
    
    try:
        os.environ['IABV_SYNAPTIC_ROUTING_ENABLED'] = 'true'
        
        print("=== E05-C25: Causalidad histórica sobre el ganador ===\n")
        ...
        # CONTROL: debe haber ventaja estática del competidor o empate con tie-break
        baseline_margin = result_control['total_git'] - result_control['total_devin']
        print(f"  CONTROL margin (git - devin): {baseline_margin}")
        
        # Si hay empate, verificar que es por tie-break alfabético
        if baseline_margin == 0.0:
            print(f"  CONTROL: empate (fit={result_control['fit_devin']}, ambos 0.0)")
            print(f"  En empate, el ganador es tie-break alfabético o orden de lista")
            print(f"  Este es un caso límite: weight puede romper el empate")
        else:
        ...
        if treatment_margin <= 0:
            print(f"\n  HALLAZGO: El historial Devin NO es suficiente para cambiar el ganador")
            ...
            print(f"  total_git = {result_treatment['total_git']}")
            print(f"  total_devin = {result_treatment['total_devin']}")
            print(f"  La ventaja fit de Git ({result_treatment['fit_git']}) supera el peso de Devin ({result_treatment['weight_devin']})")
            print(f"\nVEREDICT: NOT_PROVEN - weight_score insuficiente para superar ventaja fit_score")
            return
...
if __name__ == '__main__':
    test_e05_historical_winner_flip()
