...
def test_oses_detects_underconfidence() -> None:
    ...
    try:
        ...
        # The full review truncates findings to 8 (findings[:8]).
        # Use the internal method to verify the underconfidence
        # finding is generated by the metacognitive analysis.
        oses = bootstrap.operational_self_examination_service
        experiment_runs = bootstrap.experiment_lab_repository.list_runs(limit=50)
...
def test_oses_applies_metacognitive_feedback_on_underconfidence() -> None:
    ...
    try:
        ...
        # The full review truncates findings to 8 (findings[:8]).
        # Verify the finding is generated by the internal method.
        oses = bootstrap.operational_self_examination_service
        experiment_runs = bootstrap.experiment_lab_repository.list_runs(limit=50)
        tp_findings = oses._task_packet_pattern_findings(experiment_runs=experiment_runs)
        categories = [f.category for f in tp_findings]
