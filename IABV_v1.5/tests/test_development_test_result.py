"""
Unit tests for DevelopmentTestResult domain model.

Tests cover:
- All status types (PASSED, FAILED, ERROR, TIMEOUT, NOT_RUN)
- Invariant validation (PASSED coherence, non-negative counts, non-negative duration)
- JSON round-trip persistence
- Integration with evidence system via EvidenceKind.DEVELOPMENT_TEST
"""
...
class TestDevelopmentTestResultStatus:
    ...
    def test_failed_with_failures(self):
        """FAILED status with failed_count > 0."""
        result = DevelopmentTestResult(
    ...
    def test_error_status(self):
        """ERROR status with import error."""
        result = DevelopmentTestResult(
