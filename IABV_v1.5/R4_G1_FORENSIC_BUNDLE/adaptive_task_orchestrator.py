...
class AdaptiveTaskOrchestrator:
    ...
    # ------------------------------------------------------------------
    # G1: Goal → Plan → Tool → Capability → Protected Effect
    # ------------------------------------------------------------------

    def execute_goal_to_protected_tool(
        self,
        user_goal: str,
        *,
        ...
        """Execute a goal through the full governed path: goal → plan → tool → C2 → effect.

        This is the G1 operational entry point that demonstrates:
        1. Goal understanding via CloudReasoningPlanner
        2. Plan generation with assigned_tool
        3. Execution context registration (execution_id, run_id, session_id, episode_id)
        4. ID propagation to selected tool
        5. Real MCP tool dispatch
        6. C2 authorization (capability acquisition)
        7. Protected effect execution

        Args:
            user_goal: Natural language goal to execute
            workspace_root: Workspace root for file operations
            mcp_server: IABVMCPServer for tool dispatch (provides capability_action_bridge)

        Returns:
            Traceable result dictionary with full execution chain evidence
        """
        import uuid
        import time as _time
        ...
        # Step 1: Generate cloud plan
        _t0 = _time.monotonic()
        plan = self.generate_cloud_plan(user_goal)
        _elapsed_plan = (_time.monotonic() - _t0) * 1000

        if plan is None:
            result['status'] = 'failed'
            result['errors'].append('Cloud reasoning planner not available or failed')
            return result

        result['plan_id'] = plan.plan_id
        result['plan_summary'] = plan.summary
        result['plan_confidence'] = plan.confidence
        result['cloud_source'] = plan.cloud_source
        result['plan_generation_ms'] = round(_elapsed_plan, 2)
        result['steps_completed'].append('plan_generated')

        # Step 2: Extract assigned_tool from first step
        if not plan.steps:
            result['status'] = 'failed'
        ...
        result['assigned_tool'] = assigned_tool
        result['step_title'] = first_step.title
        result['step_description'] = first_step.description
        result['steps_completed'].append('tool_selected')

        # For G1 demo, we only support write_repo_file
        if assigned_tool != 'write_repo_file':
            result['status'] = 'rejected'
            result['errors'].append(f'G1 demo only supports write_repo_file, got: {assigned_tool}')
            return result

        # Step 3: Acquire capability using production function
        # This handles registration and lease issuance together
        invocation_id = str(uuid.uuid4())
        session_id = str(uuid.uuid4())
        episode_id = str(uuid.uuid4())
        
        # Determine target file from plan or use default
        test_file_path = 'g1_goal_execution_test.txt'
        target = f'file:{test_file_path}'
        ...
        try:
            ...
            result['status'] = 'failed'
            result['errors'].append(f'Capability acquisition failed: {e}')
            return result

        _elapsed_capability = (_time.monotonic() - _t1) * 1000

        execution_id = capability.get('execution_id')
        run_id = capability.get('run_id')
        lease_id = capability.get('lease_id')

        if not execution_id or not run_id or not lease_id:
            result['status'] = 'failed'
            result['errors'].append('Authority did not return execution_id, run_id, or lease_id')
            return result
        ...
        result['capability_acquisition_ms'] = round(_elapsed_capability, 2)
        result['steps_completed'].append('capability_acquired')

        # Step 5: Invoke tool with propagated IDs through C2
        if workspace_root is None:
            result['status'] = 'failed'
            result['errors'].append('workspace_root required for file operations')
            return result

        # Create a newAuthorityClient for CapabilityActionBridge
        # (acquire_capability_for_execution closed its connection)
        ...
        client.connect()
        try:
            capability_action_bridge = CapabilityActionBridge(authority_client=client)
            
            # Create a test file to demonstrate the full path
            test_file_path = 'g1_goal_execution_test.txt'
            test_content = f'G1 execution test\nGoal: {user_goal}\nPlan ID: {plan.plan_id}\nExecution ID: {execution_id}\nTimestamp: {datetime.now(timezone.utc).isoformat()}'

            _t3 = _time.monotonic()
            try:
                # Call write_repo_file implementation directly with C2 integration
                # This demonstrates the full path: goal → plan → tool → C2 → effect
                ...
                tool_result = write_repo_file_impl(
                    ...
                    create_dirs=True,
                    governance_fn=None,  # No additional governance needed for G1 demo
                ...
                result['status'] = 'failed'
                result['errors'].append(f'Tool dispatch failed: {e}')
                return result
        finally:
            client.disconnect()

        _elapsed_tool = (_time.monotonic() - _t3) * 1000

        result['tool_result'] = dict(tool_result) if tool_result else {}
        result['tool_dispatch_ms'] = round(_elapsed_tool, 2)
        result['steps_completed'].append('tool_invoked')

        # Step 6: Verify authorization and effect
        if result['tool_result'].get('status') != 'ok':
            result['status'] = 'failed'
            result['errors'].append(f'Tool execution failed: {result["tool_result"].get("detail")}')
            return result

        result['authorization_result'] = 'authorized'
        result['protected_effect'] = 'file_written'
        result['file_path'] = test_file_path
        result['bytes_written'] = result['tool_result'].get('bytes_written')
        result['steps_completed'].append('protected_effect_verified')

        # Step 7: Final verification
        from pathlib import Path
        full_path = Path(workspace_root) / test_file_path
        if full_path.exists():
            result['file_exists'] = True
            result['file_size'] = full_path.stat().st_size
        else:
            result['file_exists'] = False
            result['errors'].append('File was not created despite tool success')

        result['total_ms'] = round(_elapsed_plan + _elapsed_capability + _elapsed_tool, 2)
